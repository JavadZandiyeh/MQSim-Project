...
namespace SSD_Components
{
	...
	*/
	...
	public:
		...
		// 	bool use_copyback, double rho, unsigned int max_ongoing_gc_reqs_per_plane, unsigned int max_ongoing_gc_reqs_per_superblock,
		...
		// virtual void Check_gc_required_superblock(const unsigned int BlockPoolSize, const NVM::FlashMemory::Physical_Page_Address& superblockAddress) = 0;
		...
		// bool Stop_servicing_writes_superblock(const NVM::FlashMemory::Physical_Page_Address& superblock_address);
	protected:
		GC_Block_Selection_Policy_Type block_selection_policy;
		static GC_and_WL_Unit_Base * _my_instance;
		// static GC_and_WL_Unit_Base * _my_instance_superblock;
		...
		// unsigned int block_pool_gc_threshold_superblock;
		...
		// bool is_safe_gc_wl_candidate_superblock(const SuperblockBookKeepingType* sbke, const flash_block_ID_type gc_wl_candidate_block_id);//Checks if block_address is a safe candidate for gc execution, i.e., 1) it is not a write frontier, and 2) there is no ongoing program operation
		bool check_static_wl_required(const NVM::FlashMemory::Physical_Page_Address plane_address);
		// bool check_static_wl_required_superblock(const NVM::FlashMemory::Physical_Page_Address superblock_address);
		void run_static_wearleveling(const NVM::FlashMemory::Physical_Page_Address plane_address);
		// void run_static_wearleveling_superblock(const NVM::FlashMemory::Physical_Page_Address superblock_address);
		bool use_copyback;
		...
		// unsigned int block_pool_gc_hard_threshold_superblock;
		...
		// unsigned int max_ongoing_gc_reqs_per_superblock;//This value has two important usages: 1) maximum number of concurrent gc operations per superblock, and 2) the value that determines urgent GC execution when there is a shortage of flash blocks. If the block bool size drops below this value, all incomming user writes should be blocked
